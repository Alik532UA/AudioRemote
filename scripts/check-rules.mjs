...
/*
 * ПОЗНАЧКА «ДОШКОЮ КОРИСТУЮТЬСЯ» — від неї рахується вік при прибиранні, і
 * саме тому її записи перевіряються з обох боків.
 *
 * Пише її плеєр при кожному відкритті, тобто це ЄДИНЕ поле `info`, яке
 * оновлюється все життя дошки. Ціна помилки тут не «слабший захист», а
 * незворотне видалення: підроблена свіжість робить покинуту дошку вічною, а
 * підроблена давність зістарює чужу робочу.
 */
await must('господар позначає, що дошку відкривали', () =>
	write(`boards/${KEY}/info/seenAt`, SERVER_TIME, owner.token)
);

await must('дошка без seenAt лишається придатною до запису', () => {
	// Дошки, створені до появи поля, його не мають. Якби `seenAt` потрапив до
	// обов'язкових, кожен їхній наступний запис став би невалідним — тобто
	// старі дошки просто перестали б працювати.
	const legacy = info(owner.uid, 'Зал без seenAt');
	delete legacy.seenAt;
	return write(`boards/${OTHER_KEY}/info`, legacy, owner.token);
});
...
/*
 * ПІДРОБИТИ ВІК ДОШКИ — і чому це три окремі випадки, а не один.
 *
 * Від `seenAt` залежить НЕЗВОРОТНЕ видалення (`boardLifetime.ts`,
 * `scripts/sweep-boards.mjs`), тож зламати його можна у три різні боки, і
 * кожен дає свою шкоду:
 *
 *   * чужий пише свіжий час — покинута дошка стає вічною й займає місце;
 *   * господар пише час із МАЙБУТНЬОГО — те саме, але без сторонніх;
 *   * господар пише час із МИНУЛОГО — власна дошка зістарюється достроково,
 *     а це вже готовий спосіб знищити дошку, до якої немає прав на видалення.
 */
await mustNot('сторонній позначає чужу дошку як щойно відкриту', () =>
	write(`boards/${KEY}/info/seenAt`, SERVER_TIME, stranger.token)
);

await mustNot('час відкриття з майбутнього', () =>
	write(`boards/${KEY}/info/seenAt`, Date.now() + 10 * 60 * 1000, owner.token)
);

await mustNot('час відкриття з глибокого минулого', () =>
	write(`boards/${KEY}/info/seenAt`, Date.now() - 400 * 24 * 60 * 60 * 1000, owner.token)
);
