...
if (worker) {
	...
	/*
	 * І ЦЕЙ ФОЛБЕК СПРАВДІ В ПЕРЕДКЕШІ — а не лише згаданий у воркері.
	 *
	 * Перевірка вище довго була єдиною і довго була зеленою при зламаному
	 * фолбеку: рядок `/AudioRemote/404.html` у воркері стояв — як аргумент
	 * `createHandlerBoundToURL`, того самого виклику, що падав із
	 * `non-precached-url`, бо файлу в передкеші не було. Тобто вона питала
	 * «чи назвали фолбек», а питати треба «чи є ЩО віддати».
	 */
	const precached = [...worker.matchAll(/url:"([^"]+)"/g)].map((entry) => entry[1]);
	expect(
		`404.html у передкеші воркера (записів ${precached.length})`,
		precached.includes('404.html') || precached.includes(`${scope}404.html`)
	);
}
