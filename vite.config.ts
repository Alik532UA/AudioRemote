...
export default defineConfig({
	...
	test: {
		 ...
		 * ## Чому лише `.ts`
		 *
		 * Компонентних перевірок тут немає жодної, і `.svelte` дали б суцільний
		 * нуль — тобто число, яке нічого не міряє, зате ховає рух у тому, що
		 * перевірками покрите. Коли зʼявиться перша компонентна перевірка,
		 * сюди додасться другий поріг, а не розмиється цей.
		 */
		coverage: {
			provider: 'v8',
			include: ['src/lib/**/*.ts'],
