...
test('пульт помічає, що плеєр пішов', async ({ browser }) => {
	...
	/*
	 * ПЕРЕКРИТТЯ — І ВОНО НЕ МУСИТЬ БУТИ ПАСТКОЮ.
	 *
	 * Доти про мертвий пульт казав напис у колонці, а на телефоні він лежав ПІД
	 * списком: при семи віджетах до нього треба домотати. Людина бачила робочий
	 * на вигляд пульт, тиснула, нічого не діялося, і пояснення лежало там, куди
	 * вона не дивилася.
	 *
	 * Тому перевіряються ОБИДВІ половини рішення разом: вміст накритий і
	 * вимкнений для клавіатури (`inert`) — і водночас із екрана є куди піти.
	 * Перекриття, яке накрило б і смугу застосунку, було б гірше за той дефект,
	 * який воно лікує.
	 */
	await expect(remote.getByTestId('offline-hint')).toBeVisible();
	expect(
		await remote.evaluate(() => document.querySelector('main [inert]') !== null),
		'кнопки лишилися доступними з клавіатури під перекриттям'
	).toBe(true);

	await remote.getByTestId('brand').click();
	await expect(remote.getByTestId('go-create'), 'із перекритого екрана немає виходу').toBeVisible(
		ACROSS
	);
