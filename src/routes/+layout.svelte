<script lang="ts">
	...
	/**
	 * ВИСОТА СМУГИ ЗАСТОСУНКУ — числом, доступним сторінкам.
	 *
	 * Смуга липка й стоїть найвище (`z-index: 20`), і це навмисно: вихід із
	 * дошки не має ховатися ні за чим. Через те будь-яке спливне сповіщення
	 * мусить починатися ПІД нею — інакше воно накриває «Назад» і шестірню й
	 * робить із себе пастку.
	 *
...
-->
<div class="shell" style:--bar-h={barH ? `${barH}px` : null}>
	<!--
...
<style>
	...
	/*
	 * `.shell__top` тут не за компанію: скло шапки переливається на ПЕРЕМИКАННІ
	 * ТЕМИ, тобто рівно тоді ж, коли й тло сторінки. Тло в `base.css` цей
	 * перехід при зменшеному русі знімає («тема міняється миттєво — перехід тут
	 * читається як затримка малювання, а не як знак»), а шапка над ним
	 * продовжувала переливатися. Виходило, що половина екрана завмирає, а
	 * половина ні.
	 */
