...
describe('що саме лягає', () => {
	...
	/*
	 * ПЕРЕЛІК ПОЛІВ РУКАМИ ТУТ УЖЕ З'ЇДАВ ПІДПИС І СТОРОНУ: натискання доїжджало
	 * в залу без імені й без «це за пультом», тобто журнал у залі відповідав
	 * «хтось» на питання «хто». Зламатися голосно такому переліку нема де —
	 * тому питання тут не «чи є поле», а «чи доїхало ВСЕ, що поклали».
	 */
	it('натискання доїжджає цілим, а не переліком трьох полів', async () => {
		await publishPanelState('КЛЮЧ', {
			press: { cell: '4', type: 'bump', value: -5, by: 'хтось', name: 'Оля', desk: true }
		});
