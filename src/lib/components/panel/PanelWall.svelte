...
<style>
	...
	/*
	 * ПАНЕЛІ ПЕРЕНОСЯТЬСЯ, А НЕ ЇДУТЬ ЗА КРАЙ.
	 *
	 * Доти ряд мав власну горизонтальну прокрутку, і це давало два окремі
	 * незручні наслідки. Перший: щоб побачити третього помічника, доводилося
	 * крутити САМЕ цей ряд — не сторінку, а смужку всередині неї, яку ще треба
	 * було знайти мишею. Другий: панель за краєм не видно зовсім, і питання «а
	 * що там у третього» лишалося без відповіді доти, доки по неї не поїдеш.
	 *
	 * Тепер зайва панель переходить на наступний рядок, а сторінка росте вниз —
	 * і крутиться вся, як будь-яка інша. Панелі при цьому не стискаються:
	 * кнопка мусить лишатися кнопкою, у яку влучає палець.
	 */
	.wall__row {
		display: flex;
		flex-wrap: wrap;
