...
class AttentionState {
	 ...
	 * на корені — для правил, яким треба знати про спалах, і для перевірок.
	 *
	 * ФАРБУЄТЬСЯ `body`, І ЦЕ ЗАМІРЯНО, а не виведено.
	 *
	 * Спершу фарбувався `body`, потім корінь — бо тло сторінки оголошене саме
	 * на `<html>` в `app.html`, щоб вікно мало колір ще до приїзду застосунку.
	 * Міркування правильне, наслідок ні: фарба на корені на екран не виходить
	 * зовсім. Заміряно в браузері — інлайновий стиль на `<html>` стоїть,
	 * атрибут стоїть, колір вікна не міняється; той самий колір на `body`
	 * зафарбовує всю сторінку.
	 *
	 * Тому тут `body`, а `:root[data-attention]` лишається ознакою, за якою
	 * таблиця стилів фарбує `body` у протилежне тло.
	 *
