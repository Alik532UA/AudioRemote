...
/**
 * З ЯКОЇ ПІДПАПКИ ТРЕК. Порожньо — з кореня обраної папки.
 *
 * Папку з музикою складають так само, як складають будь-яку папку: «тривога»,
 * «світло», «ранкова лінійка». У списку ця межа зникала — сім треків підряд без
 * жодного натяку, що це три різні набори, і «Світло є» стояло впритул до
 * «Відбій повітряної тривоги».
 *
 * Повертається ВЕСЬ відносний шлях без імені файлу, а не лише остання ланка:
 * дві підпапки «сирена» в різних наборах — це два різні місця, і показувати їх
 * однаково означало б злити їх в одне.
 */
...
/**
 * ТИПОВИЙ ПОРЯДОК: спершу підпапка, потім назва.
 *
 * Сортування за самою назвою ігнорувало папки — і папка з наборами «тривога» та
 * «світло» перемішувалася в один список за абеткою: «Відбій повітряної тривоги»
 * ставало поруч зі «Світло є». Межу між наборами при цьому було б видно на
 * кожному другому рядку, тобто не видно взагалі.
 *
 * Кореневі треки йдуть першими — порожній рядок сортується попереду будь-якої
 * назви сам собою, і це саме те, що треба: те, що лежить просто в папці, людина
 * клала туди як головне.
 *
 * Це лише ТИПОВИЙ порядок. Щойно людина щось пересунула, порядок береться з
 * файлу налаштувань, і сюди вже не заглядають.
 */
export const byFolderThenTitle = (
	left: { path: string; title: string },
	right: { path: string; title: string }
): number =>
	folderOf(left.path).localeCompare(folderOf(right.path), 'uk') ||
