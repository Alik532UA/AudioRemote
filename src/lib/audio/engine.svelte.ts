...
export class AudioEngine {
	...
	/**
	 * НОМЕР ОСТАННЬОГО НАМІРУ. Росте на КОЖНУ команду про те, що має звучати
	 * зараз: запуск, «стоп», «пауза», «грати», вихід із дошки. Команда, якої
	 * тут немає, мовчки програє запуску, що був у дорозі, — і виглядає це як
	 * «кнопка спрацювала через раз».
	 *
	...
	pause(): void {
		...
		/*
		 * «ПАУЗА» СКАСОВУЄ Й ЗАПУСК, ЩО В ДОРОЗІ, — як «стоп» і вихід із дошки.
		 *
		 * Поки читається файл нового треку, СТАРИЙ ЩЕ ЗВУЧИТЬ: `play()` міняє
		 * `src` уже за `await`. Тобто `playing` увімкнений, і пауза доступна
		 * звідусіль — пробілом (`toggleLocal` бачить «грає»), кнопкою на плеєрі
		 * й командою з пульта.
		 *
		 * Без цього рядка вона гасила старий звук, а через мить новий трек
		 * починав грати сам: пауза ВМИКАЛА музику. Чути це рівно на великих
		 * файлах — тобто там, де читання довге й пауза найпотрібніша.
		 */
		this.intent += 1;
		/*
	...
	async resume(): Promise<void> {
		 ...
		 *
		 * Натиснули інший трек, не дочекалися й повернулися до поточного:
		 * останнє слово за «грати». Без цього рядка перемагав той трек, який
		 * натиснули РАНІШЕ, — він доїжджав із читання й сідав поверх.
		 *
		 * Рядок стоїть ЗА `cancelFade()`, а не перед ним: гасити догорілий кадр
		 * першим — окрема вимога, описана вище.
		 */
		this.intent += 1;
